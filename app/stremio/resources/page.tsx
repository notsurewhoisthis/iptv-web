...
export const metadata: Metadata = {
  title: STREMIO_CATEGORY_META.resources.title,
  description: STREMIO_CATEGORY_META.resources.description,
};

export default async function StremioResourcesPage() {
  const baseUrl = getBaseUrl();
  const articles = await getStremioArticles();
  const resources = articles.filter((a) => a.category === 'resources');
    ...
    <div className="min-h-screen py-10 bg-gradient-to-b from-gray-50 to-white">
      <BreadcrumbSchema
        items={[
          ...
          { name: 'Resources', url: `${baseUrl}/stremio/resources` },
        ]}
      />
      ...
      <div className="max-w-5xl mx-auto px-4">
        <header className="mb-8">
          ...
          <h1 className="text-3xl font-bold text-gray-900 mb-2">
            {STREMIO_CATEGORY_META.resources.title}
          </h1>
          <p className="text-gray-600">{STREMIO_CATEGORY_META.resources.description}</p>
