...
interface PageProps {
  params: Promise<{ device: string; issue: string }>;
}
...
export async function generateMetadata({ params }: PageProps): Promise<Metadata> {
  const { device, issue } = await params;
  const guide = await getDeviceTroubleshootingGuide(device, issue);
...
export default async function DeviceTroubleshootingPage({ params }: PageProps) {
  const { device: deviceId, issue: issueId } = await params;
  const guide = await getDeviceTroubleshootingGuide(deviceId, issueId);
  ...
  return (
    <div className="min-h-screen">
      ...
      <nav className="bg-gray-50 border-b border-gray-200">
        <div className="max-w-4xl mx-auto px-4 py-3">
          <ol className="flex items-center gap-2 text-sm text-gray-600 flex-wrap">
            ...
            <ChevronRight className="h-4 w-4" />
            <li>
              <Link href={`/devices/${deviceId}`} className="hover:text-gray-900">
                {guide.deviceName}
              </Link>
            </li>
            <ChevronRight className="h-4 w-4" />
      ...
      <article className="max-w-4xl mx-auto px-4 py-8">
        ...
        {guide.content.deviceSpecificTips && guide.content.deviceSpecificTips.length > 0 && (
          <section className="mb-8">
            <h2 className="text-2xl font-bold text-gray-900 mb-4">{guide.deviceName}-Specific Tips</h2>
            <div className="bg-blue-50 rounded-lg p-6">
              <ul className="space-y-2">
                {guide.content.deviceSpecificTips.map((tip, i) => (
