    ...
    <div className="min-h-screen py-10 bg-gradient-to-b from-gray-50 to-white">
      ...
      <div className="max-w-6xl mx-auto px-4">
        <header className="mb-8">
          <h1 className="text-3xl font-bold text-gray-900 mb-2">Browse by country</h1>
          <p className="text-gray-600">
            Country playlists from {data.source.name}. Availability varies by region.
          </p>
        ...
        <div className="grid sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-3">
          {data.countries.map((c) => (
            <Link
              key={c.code}
              href={`/legal-iptv/countries/${c.code}`}
            ...
            >
              ...
              <span className="ml-2 text-xs text-gray-500 uppercase">{c.code}</span>
            </Link>
          ))}
        </div>
