...
export default async function LegalIptvCategoryDetailPage({ params }: PageProps) {
  const { slug } = await params;
  const baseUrl = getBaseUrl();
    ...
    <div className="min-h-screen py-10 bg-gradient-to-b from-gray-50 to-white">
      <BreadcrumbSchema
        items={[
          ...
          { name: category.label, url: `${baseUrl}/legal-iptv/categories/${category.id}` },
        ]}
      />
      ...
      <div className="max-w-4xl mx-auto px-4">
        <header className="mb-8">
          ...
          <p className="text-gray-600">
            Category playlist URL from {data.source.name}. Use only legal streams and verify availability in your region.
          </p>
        ...
        <div className="border border-gray-200 rounded-xl p-5 bg-white">
          ...
          <div className="flex flex-col sm:flex-row gap-3 items-start sm:items-center">
            ...
            <CopyButton text={category.url} label="Copy" />
            <a
        ...
        <div className="mt-10 text-sm text-gray-600">
          <Link href="/legal-iptv/categories" className="text-blue-600 hover:underline">
            ← Back to categories
          </Link>
        </div>
      </div>
